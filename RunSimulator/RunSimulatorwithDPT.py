...
while a < 101:  # Maximum of users+ 1
    ...
    while c < 1001:
        ...
        for _ in range(10):               #number of 5ms in a period of fairness control
            # 随机选择n个元素
            ...
            while b < len(selected_elements):
                ...
                b = b + 1
            with open("DynamicProgrammingCombinedwithTimsort.py", "r") as file:
                content = file.read()
            ...
            with open("DynamicProgrammingCombinedwithTimsort_temp.py", "w") as file:
                file.write(new_content)

            with open("DynamicProgrammingCombinedwithTimsort_temp.py", "r") as file:
                content = file.read()
                # 替换DynamicProgramming0retransmission.py中的PacketSize数组
                new_content2 = content.replace("QueuingTime = []", f"QueuingTime = {QueuingTime}")
            with open("DynamicProgrammingCombinedwithTimsort_temp2.py", "w") as file:
                file.write(new_content2)

            # 运行 DynamicProgramming0retransmission_temp.py
            process = subprocess.Popen(['python', 'DynamicProgrammingCombinedwithTimsort_temp2.py'], stdout=subprocess.PIPE,
            ...
            # 删除临时文件（正确的做法）
            subprocess.run(["rm", "DynamicProgrammingCombinedwithTimsort_temp.py"])
            subprocess.run(["rm", "DynamicProgrammingCombinedwithTimsort_temp2.py"])
            # 将选择的元素的总和累加
