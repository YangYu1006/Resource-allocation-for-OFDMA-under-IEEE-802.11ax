...
while a < 101:  # Maximum of users+ 1
    ...
    while c < 1001:
        ...
        for _ in range(10):               #number of 5ms in a period of time
            ...
            selected_average_packet_size = [random.choice([1, 2]) for _ in range(a)]
            ...
            while b < len(selected_elements):
                ...
                b = b + 1
            with open("GreedyAlgorithm.py", "r", encoding='utf-8') as file:
                content = file.read()
            ...
            with open("GreedyAlgorithm_temp.py", "w", encoding='utf-8') as file:
                file.write(new_content)

            with open("GreedyAlgorithm_temp.py", "r", encoding='utf-8') as file:
                content = file.read()
                # 替换DynamicProgramming0retransmission.py中的PacketSize数组
                new_content2 = content.replace("QueuingTime = []", f"QueuingTime = {QueuingTime}")
            with open("GreedyAlgorithm_temp2.py", "w", encoding='utf-8') as file:
                file.write(new_content2)

            # 运行 DynamicProgramming0retransmission_temp.py
            process = subprocess.Popen(['python', 'GreedyAlgorithm_temp2.py'], stdout=subprocess.PIPE,
            ...
            process.communicate()
            # 删除临时文件
            subprocess.run(["rm", "GreedyAlgorithm_temp.py"])
            # 将选择的元素的总和累加
