    ...
    while c < 1001:
        ...
        for _ in range(10):               #number of 5ms in a period of time
            ...
            while b < len(selected_elements):
                ...
                b = b + 1
            with open("FIFO.py", "r", encoding="utf-8") as file:
                content = file.read()
            ...
            with open("FIFO_temp.py", "w", encoding="utf-8") as file:
                file.write(new_content)

            with open("FIFO_temp.py", "r", encoding="utf-8") as file:
                content = file.read()
                # 替换DynamicProgramming0retransmission.py中的PacketSize数组
                new_content2 = content.replace("QueuingTime = []", f"QueuingTime = {QueuingTime}")
            with open("FIFO_temp2.py", "w", encoding="utf-8") as file:
                file.write(new_content2)

            # 运行 DynamicProgramming0retransmission_temp.py
            process = subprocess.Popen(['python', 'FIFO_temp2.py'], stdout=subprocess.PIPE,
            ...
            process.communicate()
            # 删除临时文件
            subprocess.run(["rm", "FIFO_temp.py"])
            # 将选择的元素的总和累加
