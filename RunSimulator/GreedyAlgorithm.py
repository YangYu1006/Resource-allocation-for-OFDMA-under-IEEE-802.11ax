...
while k < 9:
    ...
    def knapsack_greedy_01(w_less_than_or_equal_to_26, p1, m1):
        n = len(w_less_than_or_equal_to_26)
        # compute value-per-weight but preserve indices
        value_per_weight = [(w_less_than_or_equal_to_26[i], i) for i in range(n)]
        ...
        for value, i in value_per_weight:
            if current_weight + w_less_than_or_equal_to_26[i] <= m1:
                current_weight += w_less_than_or_equal_to_26[i]
                total_value += p1[i]
                x.append(i)
            else:
                continue
    ...
    total_value, x = knapsack_greedy_01(w_less_than_or_equal_to_26, p1, m1)
