...
while k < 9:
    ...
    def knapsack_fifo(w_list, p_list, capacity):
        """
        FIFO allocation for 0-1 knapsack problem (26-tone RU).
        ...
        for i in range(len(w_list)):
            if total_weight + w_list[i] <= capacity:
                total_weight += w_list[i]
                total_value += p_list[i]
    ...
    total_value, x = knapsack_fifo(w_less_than_or_equal_to_26, p1, m1)
