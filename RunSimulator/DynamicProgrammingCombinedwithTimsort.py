...
while k < 9:
    ...
    def knapsack_dynamic(w_less_than_or_equal_to_26, p1, n, m1, x):
        for i in range(1, n + 1):
            for j in range(1, m1 + 1):
                if (j >= w_less_than_or_equal_to_26[i]):
                    optp[i][j] = max(optp[i - 1][j],
                                     optp[i - 1][j - w_less_than_or_equal_to_26[i]] + p1[i])
                else:
                    optp[i][j] = optp[i - 1][j]
                    j = m1
        for i in range(n, 0, -1):
            if optp[i][j] > optp[i - 1][j]:
                x.append(i)
                j = j - w_less_than_or_equal_to_26[i]

            # return max value which is the number in the last row and last line of the table
        v = optp[n][m1]
        return v

    print('Max Value：', knapsack_dynamic(w_less_than_or_equal_to_26, p1, n, m1, x))
    ...
    while l < len(x):
        del (PacketSize[x[l]])
        del (p1[x[l]])
        del (w_less_than_or_equal_to_26[x[l]])
        del (QueuingTime1[x[l]])
        l = l + 1
    k = k+1
    x = []
print(w_less_than_or_equal_to_26)
...
while k < NumberoftheRest52tonesRUs:
    ...
    l = 0
    def knapsack_dynamic2(w_greater_than_26, p2, n, m2, x):
        for i in range(1, n + 1):
