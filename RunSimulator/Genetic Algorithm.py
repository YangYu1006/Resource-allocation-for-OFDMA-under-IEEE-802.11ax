...
while k < 9:
    ...
    # =======================
    def knapsack_ga(w_list, p_list, capacity, pop_size=30, generations=50, mutation_rate=0.05):
        """
        Genetic Algorithm to solve 0-1 knapsack.
        - w_list: weights of items
        - p_list: values of items
        ...
        # --- Initialize population: list of chromosomes (binary vectors) ---
        population = [np.random.randint(0, 2, n).tolist() for _ in range(pop_size)]

        def fitness(individual):
            total_w = sum(w_list[i] for i in range(n) if individual[i] == 1)
            total_v = sum(p_list[i] for i in range(n) if individual[i] == 1)
            if total_w > capacity:
                return 0  # infeasible
            return total_v
        ...
        for _ in range(generations):
            ...
            # Crossover: randomly pair survivors
            children = []
    ...
    total_value, x = knapsack_ga(w_less_than_or_equal_to_26, p1, m1)
