...
while k < 9:
    ...
    def knapsack_genetic_algorithm(weights, values, capacity, population_size=50, generations=50, mutation_rate=0.1):
        n = len(weights)
        if n == 0:
            return 0, []
        ...
        def crossover(parent1, parent2):
            if n < 2:
                # Not enough loci for crossover, direct return to original sample
        ...
        selected_indices = [i for i in range(n) if best_individual[i] == 1]
        return best_value, selected_indices

    total_value, x = knapsack_genetic_algorithm(w_less_than_or_equal_to_26, p1, m1)
